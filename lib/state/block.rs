...
pub fn accumulator_diff_for_connected_body(
    ...
    let filled_transactions: Vec<_> = body
        ...
        .iter()
        .cloned()
        .map(|tx| state.fill_transaction_from_stxos(rotxn, tx))
        .collect::<Result<_, _>>()?;
    build_accumulator_diff(merkle_root, body, &filled_transactions)
}
...
pub fn utreexo_roots_after_body(
    ...
    merkle_root: MerkleRoot,
    body: &Body,
) -> Result<Vec<crate::types::UtreexoHash>, Error> {
    let filled_transactions: Vec<_> = body
        .transactions
        .iter()
        .map(|tx| state.fill_transaction(rotxn, tx))
        .collect::<Result<_, _>>()?;
    let accumulator_diff =
        build_accumulator_diff(merkle_root, body, &filled_transactions)?;
    state.accumulator_roots_after_diff(&accumulator_diff)
}

/// Validate a body-only block transition, returning fees.
#[cfg(test)]
pub fn validate(
    state: &State,
    rotxn: &RoTxn,
    ...
    let prevalidated = prevalidate(state, rotxn, header, body)?;
    check_accumulator_roots(
        state,
        &header.roots,
        [&prevalidated.accumulator_diff],
    )?;
    Ok(prevalidated.total_fees)
}

pub fn check_accumulator_roots<'a>(
    state: &State,
    expected: &[crate::types::UtreexoHash],
    diffs: impl IntoIterator<Item = &'a AccumulatorDiff>,
) -> Result<(), Error> {
    let computed_roots = state.accumulator_roots_after_diffs(diffs)?;
    if computed_roots != expected {
        return Err(Error::InvalidUtreexoRoots {
            expected: expected.to_vec(),
            computed: computed_roots,
        });
    }
    Ok(())
}
...
pub fn connect(
    ...
    state.apply_accumulator_diff(rwtxn, &prevalidated.accumulator_diff)?;
    Ok(())
}
...
mod test {
    ...
    fn disconnect_bitasset_data_update() -> anyhow::Result<()> {
        ...
        let mut genesis_header = header(None, &genesis_body);
        {
            let rotxn = env.read_txn()?;
            genesis_header.roots = state.utreexo_roots_after_body(
                &rotxn,
        ...
        let mut update_header =
            header(Some(genesis_header.hash()), &update_body);
        {
            let rotxn = env.read_txn()?;
            update_header.roots = state.utreexo_roots_after_body(
                &rotxn,
                update_header.merkle_root,
                &update_body,
            )?;
        }
